...
export default function AdminModerationDetail({ profileId }: { profileId: string }) {
  ...
  const reviewIdentityVerification = async (
    ...
    if (!note || updatingVerificationId) return;
    ...
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(result.error || "本人確認の審査結果を保存できませんでした。");
      }
          ...
          <article>
            ...
            >
              <div className={styles.sectionHeading}>
                ...
                <span>
                  最新
                  {(data.profile.identityVerificationRequests ?? []).length}件
                </span>
              </div>
              {verificationError ? (
                <p className={styles.actionError} role="alert">
              ...
              {(data.profile.identityVerificationRequests ?? []).length ? (
                <ol className={styles.reportList}>
                  {(data.profile.identityVerificationRequests ?? []).map(
                    (verificationRequest) => (
                      <li key={verificationRequest.id}>
                          ...
                          <div>
                            ...
                            <dd>
                              <time dateTime={verificationRequest.postingDeadlineAt}>
                                {formatDate(
                                  verificationRequest.postingDeadlineAt,
                                )}
                              </time>
                            </dd>
                          </div>
                        ...
                        {verificationRequest.reviewedAt ? (
                          <p className={styles.reportReviewer}>
                            審査日時: {formatDate(verificationRequest.reviewedAt)} / {verificationRequest.reviewerRole ?? "不明"} / {verificationRequest.reviewerIdentifier ?? "記録なし"}
                          </p>
                        ) : null}
                        {verificationRequest.status === "pending" ? (
                          <div className={styles.requestResolution}>
                            ...
                            <textarea
                              ...
                              value={
                                verificationNotes[verificationRequest.id] ?? ""
                              }
                              maxLength={500}
                              rows={4}
                              onChange={(event) =>
