...
const PRIVATE_PROFILE_READ_IP_LIMIT = 300;
...
const privateIpRateLimiter = new FixedWindowRateLimiter(
  PRIVATE_PROFILE_READ_IP_LIMIT,
  PROFILE_READ_WINDOW_MS,
);
...
export function consumePrivateProfileReadIpRateLimit(clientIp: string) {
  return privateIpRateLimiter.consume(clientIp);
}
