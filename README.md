...
- モデレーション対応内容の通知と既読管理
- 非公開対象の修正、公開前の管理者審査、利用停止中の修正・解除申請
- なりすまし案件における登録済みSNSを使った本人確認申請
- ヘルプページ、利用規約ページ、プライバシーポリシーページ

### 管理者向け

- 管理者ロール（`moderator` / `admin`）によるアクセス制御
- プロフィール、音声、リンク、通報、審査ケース、本人確認申請の一覧・詳細確認
- プロフィールや音声の非公開・復旧、プロフィールの利用停止
- リンクの非公開・復旧
- 修正内容と本人確認申請の審査、公開前確認
- 通報ステータス、追記型対応メモ、違反種別・確定回数の管理
- 同種2回・累計3回の確定違反による自動利用停止
- 60日間の対応期限、削除予定化、完全削除後の再登録禁止
- モデレーション履歴と審査中の証拠データの保存
- 対応時に対象ユーザーへ定型通知を作成
...
| `REPORT_TOKEN_SECRET` | サーバーのみ | 必須（本番） | 非公開後の追加通報を限定する短期署名トークン用の32文字以上のランダム値 |
...
REPORT_TOKEN_SECRET=replace-with-another-random-32-character-secret
...
`.env.local`は`.gitignore`の対象です。`DATABASE_URL`、`SUPABASE_SERVICE_ROLE_KEY`、R2の認証情報、`MODERATION_CLEANUP_SECRET`、`REPORT_TOKEN_SECRET`、`ACCOUNT_BAN_HASH_SECRET`をGitへコミットしたり、`NEXT_PUBLIC_`を付けたりしないでください。`NEXT_PUBLIC_`付きの値はブラウザへ含まれるため、秘密情報には使用できません。
...
モデレーションで非公開にした音声は、修正前後を管理者が比較できるようR2へ保持します。未完了の審査ケースに属する証拠音声は削除せず、確認済みにした時点から原則60日保持します。期限後は内部APIによる定期処理で削除します。現在のプロフィール、期限内スナップショット、または未完了ケースから参照されている音声は削除されません。R2削除に失敗した場合はDBの保存キーを残し、次回の定期処理で再試行します。
...
- Secret Manager: `DATABASE_URL`、`SUPABASE_SERVICE_ROLE_KEY`、R2認証情報、`MODERATION_CLEANUP_SECRET`、`REPORT_TOKEN_SECRET`、`ACCOUNT_BAN_HASH_SECRET`
