...
| ケース審査を承認 | ケースが審査待ちで、指定した最新スナップショットと現在の対象が一致 | 対象を公開し、ケースを`confirmed`にしてイベント、履歴、通知を作成する。削除済み対象は復元しない。アカウント停止状態は確認・解除しない |
...
| 申請を承認・却下 | 申請が`pending`で、500文字以内の回答がある | 申請を`resolved`または`rejected`にする。解除申請を承認すると、未完了ケースの有無にかかわらずプロフィールとアカウントを`active`へ戻し、管理操作履歴を作成する |
...
flowchart TD
    ...
    G["プロフィール: active<br/>アカウント: active<br/>未完了ケースが残る可能性"]
...
`postReviewPending`では管理者確認前に対象が公開されます。追加修正を求めると、対象を非公開にしてアカウントも停止しますが、停止中は修正できません。また、ケース承認と解除申請承認は互いの状態を確認しないため、未完了ケースやアカウント停止を迂回して公開状態へ戻る可能性があります。
...
公開・非公開は管理者の直接操作、ユーザー修正後の確認方式、ケース審査によって遷移します。管理者の直接復旧APIは、未完了ケースがある対象を復旧できません。一方、ケース承認APIと解除申請APIは互いの状態を確認しないため、この制限を迂回できる経路があります。リンク削除は状態を残さず行データを削除します。

公開プロフィールページは`Profile.status === "active"`だけをプロフィール全体の公開条件として確認します。`accountModerationStatus`は確認しません。プロフィールが公開されている場合、音声は`audioStatus === "active"`、リンクは各リンクの`status === "active"`のものだけを表示します。そのため、`accountModerationStatus`が`suspended`でも`Profile.status`が`active`へ戻ると、現行実装では外部公開されます。
...
| `suspended` | 解除申請を承認 | 管理者 | `active` | 未完了ケースを確認せずプロフィールとアカウントを公開し、申請を`resolved`にして復旧履歴を作成する |
...
この節は、[issue #102](https://github.com/gp-kobayashi/oto_meishi/issues/102)で決定した変更後の仕様です。現行実装の説明ではありません。各項目は関連PRで段階的に実装し、実装が完了した項目から該当する現行仕様へ移します。
...
- 解除申請の承認は未完了ケースを確認せず、プロフィールとアカウントを`active`へ戻す。
- ケース承認はアカウント停止状態を確認せず、個別対象を`active`へ戻す。
- 公開プロフィールページは`accountModerationStatus`を確認しない。
...
これらは[issue #102](https://github.com/gp-kobayashi/oto_meishi/issues/102)の関連PRで修正します。修正が完了するまで、管理者は未完了ケースとアカウント状態を手動で確認し、解除・承認・証拠削除を慎重に行う必要があります。
