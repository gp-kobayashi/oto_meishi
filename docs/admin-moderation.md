...
| 本人確認申請を承認・却下 | 申請が審査待ちで、投稿期限内かつ500文字以内の説明がある | 承認時はなりすましケースを完了して対応する違反回数を取り消す。取消対象が停止を発生させた違反で、他の停止理由や削除処理がなければアカウント停止も訂正し、対象コンテンツを個別条件に従い再公開する。審査結果とメモは常に保存し、状態が実際に変わった場合だけ対応履歴と通知を作成する。却下時は非公開と修正待ちを維持し、対応履歴と通知を作成する。期限切れ申請は`expired`にして審査しない。 |
...
- 管理者確認待ちまたは解除申請確認中は、利用者側の放置ではないため期限を進めない。定期処理ではバッチ取得前にこれらを除外し、後続の期限超過対象を滞留させない。

完全削除では、処理対象を原子的に確保し、削除予定のR2キーを一時記録してから、プロフィールに紐づく利用データ、他アカウントに共有されていないR2オブジェクト、Supabase Authユーザーの順で削除します。R2またはAuth削除に一時的に失敗した場合は、利用データを復元せず、次回の期限処理で未完了の外部削除を再試行します。すべて完了した後に一時記録したR2キーを消去します。処理結果では`deletionCandidates`、`deleted`、`pendingAuthDeletionsCompleted`、`failed`を確認できます。
...
- 通常のケース承認は対象を確認済みにしても、アカウント停止を解除しない（本人確認成功時の停止状態訂正は別経路）。
...
違反の確定と取り消しは、過去の記録を更新・削除せず、新しい履歴として追記します。DBトリガーにより通常操作での違反履歴の更新・削除を禁止し、利用者データの完全削除トランザクション内でのみ削除を許可します。

### 本人確認と著作権確認

なりすましの本人確認では身分証明書を収集しません。利用者は編集画面から登録済みSNSと投稿予定内容を事前に申請し、申請から10分以内に一致する任意の文章または写真を投稿します。申請時のSNS URL、投稿予定内容、投稿期限は履歴として保存されます。管理者は期限内の申請に限って申請内容とSNS投稿を目視で照合し、審査結果と説明を保存します。期限を過ぎた申請は`expired`となり、管理者は承認または却下できません。

本人と確認できた場合は、なりすましケースを確認済みにし、対応する違反回数を取り消します。取消対象の違反が停止を発生させたものの場合だけ、取り消し後の有効な確定違反を再集計します。即時停止理由、同種2件、累計3件のいずれかが残る場合、削除保留・削除処理中の場合、または停止を発生させた違反でない場合は、利用停止を維持します。停止理由がなくなった場合だけアカウントを`active`へ訂正し、保留中の解除申請を自動解決します。本人と確認できない場合は非公開と修正待ちを維持し、その理由をサービス内通知で利用者へ伝えます。

本人確認後の対象コンテンツ再公開は、アカウント状態が`active`で、同一対象に未完了ケースがない場合だけ行います。プロフィールは`status`を`active`へ、非削除音声は`audioStatus`を`active`へ、指定されたプロフィール所属の非公開リンクだけを`active`へ戻します。本人確認対象が音声・リンクの場合でも、停止状態の訂正後にプロフィール表面が`suspended`で、他のプロフィール対象ケースが未完了でなければプロフィールを`active`へ戻します。`hidden`のプロフィールは自動で公開しません。削除済み音声、存在しないリンク、別プロフィールのリンク、証拠スナップショットだけのリンクは復元しません。アカウント停止の訂正とコンテンツ再公開は別の操作履歴・通知として、実際に状態が変わった場合だけ記録します。

本人確認審査と解除申請の処理は、同一プロフィールについてプロフィールロックを先に取得し、その後に申請ロックを取得します。ロック後に状態を再取得するため、先に処理された操作がある場合は409で終了し、履歴や通知を重複作成しません。
