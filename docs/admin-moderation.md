...
このドキュメントは、oto_meishiの管理者機能について、現在の実装とテストで確認できる仕様をまとめたものです。将来の運用案は「未対応・既知の制約・検討事項」に分離し、実装済みの機能と混同しないようにします。
...
| ケース審査を承認 | ケースが審査待ちで、指定した最新スナップショットと現在の対象が一致 | 対象を公開し、ケースを`confirmed`にしてイベント、履歴、通知を作成する。削除済み対象は復元しない |
...
| 申請を承認・却下 | 申請が`pending`で、500文字以内の回答がある | 申請を`resolved`または`rejected`にする。解除申請の承認時だけアカウントを`active`へ戻し、管理操作履歴を作成する |
...
APIは現在と異なる状態への変更を受け付けるため、UIを経由しない場合は`pending`から`resolved`または`dismissed`へ直接変更できます。通報状態の変更だけではユーザー通知を作成しません。非公開や利用停止を別途実行した場合に、その管理操作の通知を作成します。
...
`harassment`、`impersonation`、`other`は`preReview`です。それ以外の`inappropriateContent`、`copyrightConcern`、`unsafeLink`、`serviceMismatch`は`postReview`です。管理APIが受け付けるのはこれらの定義済み理由だけです。内部の確認方式判定は、未知の値を受け取った場合に安全側の`preReview`を返す設計です。
...
公開・非公開は管理者の直接操作、ユーザー修正後の確認方式、ケース審査によって遷移します。未完了ケースがある対象は直接復旧できず、ケース審査の承認からのみ再公開できます。リンク削除は状態を残さず行データを削除します。
...
| `suspended` | 解除申請を承認 | 管理者 | `active` | プロフィールを公開し、申請を`resolved`にして復旧履歴を作成する |
| `suspended` | 解除申請を却下 | 管理者 | `suspended`のまま | 申請を`rejected`にして回答を保存する |

停止中はプロフィール・音声・リンクを変更できません。未処理の解除申請は同時に1件だけ作成できます。`deletionPending`と`deletionScheduledAt`はDB定義と申請拒否判定には存在しますが、そこへ遷移させる管理処理、期限経過処理、完全削除処理は未実装です。
