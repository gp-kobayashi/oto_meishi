...
停止中はプロフィール・音声・リンクを変更できません。未処理の解除申請は同時に1件だけ作成できます。`deletionPending`への期限経過処理は実装済みですが、削除予定日を過ぎたアカウントの完全削除と再登録防止は未実装です。
...
- 削除予定日を過ぎたプロフィールを削除候補として処理結果へ含める。
- 利用停止と削除予定化の際に、システム実行の監査履歴、ケースイベント、ユーザー通知を保存する。
- 管理者確認待ちまたは解除申請確認中は、利用者側の放置ではないため期限を進めない。

完全削除と再登録防止は別工程です。現在の期限APIは削除予定日を過ぎたプロフィールを`deletionCandidates`として返しますが、プロフィールやAuthユーザーを削除しません。
