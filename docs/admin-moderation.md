...
本人確認後の対象コンテンツ再公開は、アカウント状態が`active`で、同一対象に未完了ケースがない場合だけ行います。プロフィールは`status`を`active`へ、非削除音声は`audioStatus`を`active`へ、指定されたプロフィール所属の非公開リンクだけを`active`へ戻します。削除済み音声、存在しないリンク、別プロフィールのリンク、証拠スナップショットだけのリンクは復元しません。アカウント停止の訂正とコンテンツ再公開は別の操作履歴・通知として、実際に状態が変わった場合だけ記録します。
